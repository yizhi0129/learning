...
(select dno, count(dno) as total from TbEmp group by dno) t2 on deptno=dno;
...
-- 写法1:
select ename, sal, t1.dno from TbEmp as t1, (select dno, avg(sal) as avgSal from TbEmp 
group by dno) as t2 where t1.dno=t2.dno and t1.sal>t2.avgSal;
-- 写法2:
select ename, sal, t1.dno from TbEmp as t1 inner join (select dno, avg(sal) as avgSal from TbEmp 
group by dno) as t2 on t1.dno=t2.dno and t1.sal>t2.avgSal;

-- 查询部门中薪水最高的人姓名、工资和所在部门名称
select ename, sal, dname from TbDept as t1 inner join (select ename, sal, t2.dno from TbEmp as t2 
inner join (select dno, max(sal) as maxSal from TbEmp group by dno) t3
